...
public class AS2SenderModule extends HttpSenderModule {
    ...
    public void handle(String action, Message msg, Map<Object, Object> options) throws OpenAS2Exception {
        ...
        try {
            try {
                ...
                logger.error("Unexpected error sending file: ", e);
                resend(msg, new OpenAS2Exception(e), retries);
                return;
            }
            // Receive an MDN
            if (msg.isConfiguredForMDN()) {
                ...
                if (msg.getPartnership().getAttribute(AS2Partnership.PA_AS2_RECEIPT_OPTION) == null) {
                    ...
                    try {
                        ...
                        logger.warn("Failed to get input stream for receiving MDN: ", e1);
                        resend(msg, new OpenAS2Exception(e1), retries);
                    }
                    ...
                    try {
                        ...
                        // What to do???
                        resend(msg, new OpenAS2Exception(ioe), retries);
                    } finally {
                        try {
                            if (connIn != null)
                                connIn.close();
                    ...
                    try {
                        ...
                        if (Message.MSG_STATUS_MDN_PROCESS_INIT.equals(msg.getStatus()) || Message.MSG_STATUS_MDN_PARSE.equals(msg.getStatus()) || !(e instanceof OpenAS2Exception)) {
                            /*
                             * Cannot identify the target if in init or parse state so not sure what the best course of action is apart from
                             * do nothing
                             */
                            logger.error("Unhandled error condition receiving asynchronous MDN. Message and asociated files cleanup will be attempted but may be in an unknown state.", e);
                        }
                        /*
                         * Most likely a resend abort of max resend reached if OpenAS2Exception so do not log as should have been logged
                         * upstream ... just clean up the mess
                         */
                        else {
                            // Must have received MDN successfully
                            logger.error("Exception receiving asynchronous MDN. Message and asociated files cleanup will be attempted but may be in an unknown state.", e);
    ...
    protected void checkRequired(Message msg) throws InvalidParameterException {
        ...
        try {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "ContentType", msg.getContentType());
            }
            InvalidParameterException.checkValue(msg, "Attribute: " + AS2Partnership.PA_AS2_URL, partnership.getAttribute(AS2Partnership.PA_AS2_URL));
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Attribute: " + AS2Partnership.PA_AS2_URL, partnership.getAttribute(AS2Partnership.PA_AS2_URL));
            }
            InvalidParameterException.checkValue(msg, "Receiver: " + AS2Partnership.PID_AS2, partnership.getReceiverID(AS2Partnership.PID_AS2));
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Receiver: " + AS2Partnership.PID_AS2, partnership.getReceiverID(AS2Partnership.PID_AS2));
            }
            InvalidParameterException.checkValue(msg, "Sender: " + AS2Partnership.PID_AS2, partnership.getSenderID(AS2Partnership.PID_AS2));
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Sender: " + AS2Partnership.PID_AS2, partnership.getSenderID(AS2Partnership.PID_AS2));
            }
            InvalidParameterException.checkValue(msg, "Subject", msg.getSubject());
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Subject", msg.getSubject());
            }
            InvalidParameterException.checkValue(msg, "Sender: " + Partnership.PID_EMAIL, partnership.getSenderID(Partnership.PID_EMAIL));
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Sender: " + Partnership.PID_EMAIL, partnership.getSenderID(Partnership.PID_EMAIL));
            }
            InvalidParameterException.checkValue(msg, "Message Data", msg.getData());
            if (logger.isTraceEnabled()) {
                logger.trace("Check required [{}] : {}", "Message Data", msg.getData());
            }
    ...
    protected MimeBodyPart secure(Message msg) throws Exception {
        ...
        /*
         * Based on RFC4130, RFC6362 and RFC5042, the MIC is calculated as follows: Signed message - MIME header fields and content that is
         * to be signed which may or may not be encrypted and/or compressed.
         * 
         * Unsigned encrypted message - data content including all MIME header fields and any applied Content-Transfer-Encoding prior to
         * encryption and/or compression
         * 
         * So essentially, calculate the MIC before doing any compression or encryption if message not being signed otherwise calculate
         * right before signing of the message but include headers for unsigned messages (see RFC4130 section 7.3.1 for details)
         */
        ...
        if (isCompress && isCompressBeforeSign) {
            ...
            if (!sign && !encrypt) {
                logger.info("No Signature and no encryption needed");
                // Add any additional headers since this will be the outermost Mime body part if configured
        ...
        if (sign) {
            logger.info("Signature required");
        ...
        if (encrypt) {
            logger.info("Encryption required");
            // Add any additional headers since this will be the outermost Mime body part if configured
            addCustomOuterMimeHeaders(msg, dataBP);
            String algorithm = partnership.getAttribute(SecurePartnership.PA_ENCRYPT);
            logger.trace("Encryption algorithm [{}]", algorithm);
            ...
            logger.trace("Encrypted multipart [{}]", MimeUtil.toString(dataBP, true));
    ...
    /**
     * @description Stores metadata into pending information file and storing message object from first send attempt. The message object is
     *              written to a separate file to avoid repeated rewrites of possibly very large objects since it contains the original file
     *              data
    ...
    protected void storePendingInfo(AS2Message msg, boolean isResend) throws Exception {
        ...
        try {
            ...
            String pendingFile = isResend ? msg.getAttribute(FileAttribute.MA_PENDINGFILE) : AS2Util.buildPendingFileName(msg, getSession().getProcessor(), "pendingmdn");
            ...
            throw new Exception("Unable to set up pending information files.", e);
        } finally {
            if (oos != null)
                try {
                    oos.close();
    ...
    protected void calcAndStoreMic(Message msg, MimeBodyPart mbp, boolean includeHeaders) throws Exception {
        ...
        DispositionOptions dispOptions = new DispositionOptions(msg.getPartnership().getAttribute(AS2Partnership.PA_AS2_MDN_OPTIONS));
